...
export default class ScreenDefinitionInterpreter extends PerspectivesComponent<ScreenDefinitionInterpreterProps, ScreenDefinitionInterpreterState>
{
  ...
  screenElement(taggedElement : ScreenElementDefTagged, index : number)
  {
    ...
    switch (taggedElement.elementType){
      case "RowElementD":
        return component.buildRow( taggedElement.elementDef as RowElementDef, index );
      case "ColumnElementD":
        return component.buildColumn( taggedElement.elementDef as ColumnElementDef, index );    
      case "TableElementD":
        const tableDef = taggedElement.elementDef as TableElementDef;
        return (
          <div
            className="border-bottom pb-4 pt-4 widget"
            key={index}
            >
          { tableDef.table.title ? <h4>{tableDef.table.title}</h4> : null }
          { component.buildTable( tableDef ) }
          </div>);
      case "FormElementD":
        const formDef = taggedElement.elementDef as FormElementDef;
        return (
          <div
            className="border-bottom pb-4 pt-4 widget"
            key={index}
            >
          { formDef.form.title ? <h4>{formDef.form.title}</h4> : null }
          { component.buildForm( formDef ) }
          </div>);
      case "MarkDownElementD":
        const markDownDef = taggedElement.elementDef as MarkDownElementDef;
        return (
          <div 
            key={index}
      ...
      case "ChatElementD":
        const chatDef = taggedElement.elementDef as ChatElementDef;
        return (
          <div
            key={index}
  ...
  buildTable({table} : TableElementDef)
  {
    const perspective = table.perspective;
  ...
  buildForm({form} : FormElementDef)
  {
    const component = this;
    const perspective = form.perspective;
    // const title = widgetCommonFields.title;
    return (
